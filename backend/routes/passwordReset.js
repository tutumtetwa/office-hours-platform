    ...
    try {
        ...
        if (!sent) {
            return res.status(500).json({ error: 'Failed to send email. Please try SMS.' });
        }
        res.json({ 
    ...
    // Email sending function - supports SendGrid or SMTP
    async function sendEmailCode(email, firstName, code) {
    console.log(`Attempting to send email code to ${email}`);
    ...
    if (process.env.SENDGRID_API_KEY) {
        ...
        await sgMail.send({
            to: email,
            from: process.env.SMTP_FROM || process.env.SENDGRID_FROM || 'noreply@officehours.app',
    ...
    if (process.env.SMTP_HOST) {
        ...
        await transporter.sendMail({
            from: process.env.SMTP_FROM || 'noreply@officehours.app',
            to: email,
            subject: 'Password Reset Code - Office Hours',
            html: getEmailTemplate(firstName, code)
        });
    ...
    console.log(`═══════════════════════════════════════════`);
    console.log(`  [DEV MODE] PASSWORD RESET CODE`);
    console.log(`  Email: ${email}`);
    console.log(`  Code:  ${code}`);
    console.log(`═══════════════════════════════════════════`);
    return true; // Return true in dev mode so user can check logs
    }

    // SMS sending function - uses Twilio
    async function sendSMSCode(phone, code) {
    console.log(`Attempting to send SMS code to ${phone}`);
    
    if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_PHONE_NUMBER) {
        console.log(`═══════════════════════════════════════════`);
        console.log(`  [DEV MODE] SMS RESET CODE`);
    ...
    return `
        ...
        <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #1e3a5f; margin: 0;">📅 Office Hours</h1>
        </div>
